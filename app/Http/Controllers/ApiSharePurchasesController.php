...
class ApiSharePurchasesController extends Controller
{
    ...
    {
        $user = Auth::user();
        // grab from the request only the fields needed
        ...
        // some validation in the backend so we can be sure 
        // the data is right
        ...
        // even more elegant we could have done
        $share = $user->shares()->create($data);
    ...
    public function saveShareTags(Request $request, $shareId)
    {
        ...
        // Loop through the tags coming from the request to look for 
        // tags that need to be stored in the tags table
        foreach ($data as $key => $tagArr) {
            if (!array_key_exists('id', $tagArr)) {
                $storeTags[] = $user->tags()->create($tagArr)->id;
            } else {
                $storeTags[] = $tagArr['id'];
            }
        }
